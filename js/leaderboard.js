...
async function ensureWeekIsCurrent() {
  ...
  if (!isFirstEverCheck) {
    try {
      ...
      // If the ranked query fails (e.g. a missing Firestore index),
      // just carry the player's rank over unchanged rather than block
      // the leaderboard from loading at all.
    }
  }
...
async function renderLeaderboard() {
  ...
  const changeBanner = currentUserData.lastRankChange === "up"
    ? `<p class="rank-change rank-change-up">⬆️ Promoted to ${rankInfo.name} last week!</p>`
    : currentUserData.lastRankChange === "down"
      ? `<p class="rank-change rank-change-down">⬇️ Demoted to ${rankInfo.name} last week.</p>`
      : "";
  currentUserData.lastRankChange = null; // only show the banner once
