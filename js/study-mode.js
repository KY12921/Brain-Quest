...
async function finishStudyModeLesson() {
  ...
  // Same reasoning as finishLesson in app.js: "1 full quest" should
  // mean the whole chapter, not a single lesson within it.
  const chapterStepKeys = steps.filter(s => s.chapterIndex === step.chapterIndex).map(s => getStudyModeStepKey(s));
