...
function renderProScreen() {
  ...
  if (isUltra) {
    card.innerHTML = `
      <p class="pro-status pro-active">🌟 You have version5 Ultra</p>
      ${comparisonTable}
    ...
    card.innerHTML = `
      <p class="pro-status pro-active">✅ You have version5 Pro</p>
      ${comparisonTable}
      ${billingToggle}
    ...
    card.innerHTML = `
      ...
      <button class="btn btn-ghost" id="ultra-activate-btn">Activate Ultra (test, no real payment)</button>
      ${coinPackSection}
    `;
    ...
    document.getElementById("ultra-activate-btn").addEventListener("click", () => setTierStatus(true, true));
    wireBillingToggle();
  }
