...
function renderBgThemePicker() {
  ...
  grid.innerHTML = BG_THEMES.map(theme => `
    <button class="bg-theme-card${theme.id === current ? " bg-theme-card-active" : ""}" data-theme-id="${theme.id}">
      <span class="bg-theme-swatch bg-theme-swatch-${theme.id}"></span>
      <p class="bg-theme-name">${theme.name}</p>
      <p class="bg-theme-desc">${theme.description}</p>
    </button>
  `).join("");
