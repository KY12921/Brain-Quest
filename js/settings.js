...
function renderSettingsScreen() {
  ...
  renderOllieVoicePicker();
}

function renderOllieVoicePicker() {
  const style = (currentUserData && currentUserData.ollieVoiceStyle) || "soft";
  document.querySelectorAll('input[name="ollie-voice"]').forEach(radio => {
    radio.checked = radio.value === style;
    const label = radio.closest(".ollie-voice-option");
    if (label) label.classList.toggle("ollie-voice-option-active", radio.checked);
  });
}
...
document.querySelectorAll('input[name="ollie-voice"]').forEach(radio => {
  radio.addEventListener("change", () => {
    ...
    currentUserData.ollieVoiceStyle = style;
    renderOllieVoicePicker(); // refresh which option shows as active
    if (currentUser) {
      db.collection("users").doc(currentUser.uid).update({ ollieVoiceStyle: style });
    }
    // A quick sample so switching voices actually feels different,
    // rather than just changing a setting with no immediate feedback.
    if (style !== "off") {
      speakAsOllie("Hi! This is what I sound like now.");
    }
  });
});
