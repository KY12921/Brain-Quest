...
// Fill-in-the-blank: a genuinely different test than picking from
// given options — the student has to produce the answer themselves,
// not just recognize it among distractors. question.acceptedAnswers
// is an array so reasonable variants all count (e.g. ["paris"] or
// ["4", "four"]) — checked case-insensitively and trimmed, since
// neither of those should ever be what makes an otherwise-correct
// answer count as wrong.
function renderFillBlankWidget(question) {
  document.getElementById("quiz-options").classList.add("hidden");
  const container = document.getElementById("interactive-widget-container");
  container.classList.remove("hidden");

  container.innerHTML = `
    <div class="fill-blank-widget">
      <input type="text" id="fill-blank-input" class="fill-blank-input" placeholder="Type your answer…" autocomplete="off" autocapitalize="off" spellcheck="false">
    </div>
  `;

  const input = document.getElementById("fill-blank-input");
  input.addEventListener("input", () => {
    document.getElementById("quiz-confirm-btn").classList.toggle("hidden", input.value.trim() === "");
  });
  // Enter submits, same as tapping Confirm — a text input is the one
  // widget here where that's the obviously expected behavior.
  ...
  window.__interactiveCheck = () => {
    ...
    const accepted = (question.acceptedAnswers || []).map(a => String(a).trim().toLowerCase());
    const isCorrect = accepted.includes(typed);
    return { isCorrect, pickedText: input.value.trim() };
  };
}
