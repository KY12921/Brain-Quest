...
function renderSubjects() {
  ...
  SUBJECTS.forEach(subject => {
    ...
    if (hasRoadmap) {
      ...
      metaText = completedCount === lessons.length ? "Roadmap complete ✅" : `Lesson ${unlockedCount} of ${lessons.length}`;
    } else {
...
function renderRoadmap(subject) {
  document.getElementById("roadmap-heading").textContent = `${subject.name} Roadmap`;
  const lessons = LESSONS[subject.id];
  const unlockedCount = (currentUserData.lessonProgress && currentUserData.lessonProgress[subject.id]) || 1;
