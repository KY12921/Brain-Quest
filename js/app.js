...
auth.onAuthStateChanged(async (user) => {
  ...
  if (user) {
    ...
    currentUserData = {
      ...
      ollieVoiceStyle: data.ollieVoiceStyle || "soft",
...
// ------------------------------------------------------------------
// Ollie's voice — uses the browser's built-in text-to-speech
// (Web Speech API), not a recorded voice or an external service, so
// it works with no extra setup and no per-use cost. "Soft" and
// "deep" are achieved via pitch/rate rather than picking a named
// system voice, since available voices vary wildly by browser and OS
// and aren't reliably selectable by character — pitch and rate are
// universally supported and give a genuinely different feel.
// ------------------------------------------------------------------
function speakAsOllie(text) {
  if (!window.speechSynthesis || !text) return;
  const voiceStyle = (currentUserData && currentUserData.ollieVoiceStyle) || "soft";
  if (voiceStyle === "off") return;

  window.speechSynthesis.cancel(); // don't let overlapping hints talk over each other
...
async function showOllieHelper(explanationFallback) {
  ...
  speakAsOllie("Hey, looks like this one's tricky — let me help.");
...
document.getElementById("ollie-hint-btn").addEventListener("click", async () => {
  ...
  speakAsOllie("Hmm, let me think about that.");
  btn.disabled = true;
