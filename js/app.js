...
// shared quiz engine used by Quests and Duel modes.
// ------------------------------------------------------------------
...
// Kiwi, given a genuinely more dimensional look via layered radial/
...
let currentUserData = null; // { name, xp, isPro, completedSubjects, mission }
let currentQuiz = null; // { mode: 'quest'|'duel', subjectId, subjectName, questions, index, correctCount, wrongCount, duelId, duelRole }
...
function renderHome() {
  ...
  renderKiwiMascotInto(document.getElementById("home-mascot"), 72);
...
function renderStreakCalendar() {
  ...
  // A few hollow, uncolored cells for the days just ahead of today —
  // not part of the streak yet, just showing what's available to keep
  // it going. Placed before (to the left of) today, consistent with
  // this strip's existing "today first, history trailing right"
  // order — the newest thing shown here is furthest from history in
  // either direction. Rendered furthest-future-first so appending
  // them in this order lands nearest-future closest to today's cell.
  const upcomingDaysToShow = 3;
  for (let i = upcomingDaysToShow; i >= 1; i--) {
    const d = new Date(Date.now() + i * 86400000);
    const cell = document.createElement("div");
    cell.className = "streak-day streak-day-upcoming";
    cell.innerHTML = `<span class="streak-day-label">${dayLabels[d.getDay()]}</span><span class="streak-day-dot"></span>`;
    strip.appendChild(cell);
  }

  // Show enough days to actually display the current streak, not a
  // fixed 7 — a 15-day streak was rendering identically to a 7-day
  // one, since this always drew exactly 7 cells regardless of
  // streakCount. Still shows at least 7 (a full week) so new users
  // get the familiar weekly view, capped at 30 since that's as far
  // back as activeDates itself is retained (see updateStreak above).
  // Same ordering as before — i=0 (today) first/leftmost, older days
  // trailing to the right — just no longer hardcoded to stop at 6.
  const daysToShow = Math.max(7, Math.min(currentUserData.streakCount || 0, 30));
...
document.getElementById("guest-mode-btn").addEventListener("click", async () => {
  ...
  try {
    ...
    // auth/operation-not-allowed / auth/admin-restricted-operation means
    // the Anonymous sign-in provider hasn't been turned on for this
    // Firebase project (Authentication > Sign-in method > Anonymous) —
    // that's a project setting, not something this code can fix, so
    // say so plainly instead of a generic "try again" that just masks
    // a config problem the developer needs to go address.
    const errorEl = document.getElementById("signin-error");
    if (err.code === "auth/operation-not-allowed" || err.code === "auth/admin-restricted-operation") {
      errorEl.textContent = "Guest mode isn't turned on for this app yet — the site owner needs to enable Anonymous sign-in in Firebase.";
    } else {
      errorEl.textContent = `Couldn't start guest mode right now (${err.code || err.message || "unknown error"}). Please try again.`;
    }
    btn.disabled = false;
  }
});
...
function renderRoadmap(subject, highlightLessonIndex) {
  ...
  steps.forEach((step, i) => {
    ...
    // Not-yet-reached lessons are no longer hard-disabled — you can
    // pick any lesson you like (see the click handler below), you
    // just get a heads-up first if you're skipping ahead of where
    // you'd normally be, since later lessons can build on earlier
    // ones. Pro-locking is a separate, genuine gate and still
    // disables the button outright.
    const isSkipAhead = !isReached;
    const isLocked = isProLocked;
    const side = i % 2 === 0 ? "left" : "right";

    const node = document.createElement("div");
    node.className = `roadmap-node roadmap-node-${side}${step.isChallenge ? " roadmap-node-challenge" : ""}`;

    const btn = document.createElement("button");
    btn.className = `roadmap-dot${isCompleted ? " seal-badge roadmap-dot-done" : ""}${isSkipAhead ? " roadmap-dot-skip-ahead" : ""}${isProLocked ? " roadmap-dot-pro-locked" : ""}${step.isChallenge ? " roadmap-dot-challenge" : ""}`;
    btn.innerHTML = isCompleted ? UTIL_ICON_SVG.check : (isProLocked ? UTIL_ICON_SVG.lock : (step.isChallenge ? "⚡" : String(i + 1)));
    btn.disabled = isLocked;
    btn.title = isProLocked ? "Requires Pro" : (isSkipAhead ? "Not yet reached — click to skip ahead" : "");
    if (isProLocked) {
      btn.addEventListener("click", () => navigateTo("pro-section"));
    } else if (isSkipAhead) {
      btn.addEventListener("click", () => confirmSkipAheadThenStart(subject, i));
    } else {
      btn.addEventListener("click", () => showLessonIntro(subject, i));
    }
    ...
    if (i === highlightLessonIndex) {
      ...
      renderKiwiMascotInto(mascot, 60);
...
// This previously used a custom full-screen overlay (mascot, message,
// two buttons) instead of a native dialog, for a nicer, on-brand look.
// It got reported stuck — screen darkened, nothing clickable — twice,
// and I couldn't pin an exact cause through code review or simulated
// testing (the logic checked out both times; something about the real
// rendering/CSS/event environment didn't match what I could verify
// remotely). Rather than ship a third attempt at the same custom
// overlay on unverified confidence, this now uses the browser's own
// confirm() dialog: it's rendered entirely outside the page's own
// CSS and DOM, so there is no overlay z-index, no click-target
// element, no CSS transition, and no custom event wiring that could
// ever leave it stuck — it is a native, browser-guaranteed control.
// The trade-off is losing the Kiwi mascot and custom styling here;
// that's an acceptable cost for a confirmation step actually working
// every time.
function confirmSkipAheadThenStart(subject, lessonIndex) {
  const proceed = confirm("Heads up — this skips ahead of where you left off. You might miss something the later lesson builds on. Continue anyway?");
  if (proceed) {
    showLessonIntro(subject, lessonIndex);
  }
}
...
function beginLessonQuiz(subject, lessonIndex) {
  ...
  // fixed, unlike Trivia's larger pool) — an acceptable trade-off for
  // honoring the Settings toggle rather than fabricating a
  // substitute question on the fly.
...
async function finishLesson() {
  ...
  // "Complete 1 full quest" means the whole chapter (every lesson
  // within it), not just this one step — registering the mission
  // event on every single lesson finish (as this used to do) let
  // finishing just "Part 1" of a 3-lesson chapter satisfy a mission
  // that says "full quest," which is exactly wrong. Only count it
  // once every step in this chapter is complete.
  const chapterStepKeys = steps.filter(s => s.chapterIndex === step.chapterIndex).map(s => getStepKey(subjectId, s));
  const chapterNowComplete = chapterStepKeys.every(k => currentUserData.completedSubjects.includes(k));
  if (chapterNowComplete) registerMissionEvent("questsCompleted", 1);
...
function renderQuestion() {
  ...
  // Concept-first only makes sense for learning-oriented modes. Live
  // Battles are meant to feel fast-paced and competitive — showing a
  // mini-lesson above every question there would work against what
  // that mode is for, so it skips straight to the question like
  // before.
...
function renderQuestionCore() {
  ...
  document.getElementById("kiwi-hint-panel").classList.add("hidden");
  ...
  if (question.type === "balance") {
    ...
    renderFillBlankWidget(question);
  } else if (question.type === "true-false") {
    // Deliberately still uses question.options (["True", "False"],
    // authored on the question itself) rather than hardcoding the
    // labels here — hints, explanations, and the wrong-answer
    // feedback text all read question.options[selectedIndex]
    // downstream, and giving true/false questions a real options
    // array means all of that keeps working with no special-casing
    // anywhere except this render step's styling.
    const optionsEl = document.getElementById("quiz-options");
    optionsEl.innerHTML = "";
    optionsEl.classList.add("quiz-options-true-false");
    question.options.forEach((optionText, i) => {
      const btn = document.createElement("button");
      btn.className = "quiz-option quiz-option-true-false";
    ...
    const optionsEl = document.getElementById("quiz-options");
    optionsEl.innerHTML = "";
    optionsEl.classList.remove("quiz-options-true-false");
    question.options.forEach((optionText, i) => {
      const btn = document.createElement("button");
...
document.getElementById("quiz-confirm-btn").addEventListener("click", () => {
  ...
  if (question.type === "balance" || question.type === "slope-drag" || question.type === "sequence" || question.type === "grid-logic" || question.type === "sentence-build" || question.type === "speak-sentence" || question.type === "fill-blank") {
...
function xpPerCorrectForMode(mode) {
  ...
  if (mode === "duel") base = DUEL_XP_PER_CORRECT;
...
function handleAnswer(selectedIndex) {
  ...
  // correct answer stays hidden, and Kiwi's hint becomes available.
...
// shown the answer, and offers Kiwi's hint (not the answer itself)
...
// answer — XP, mission tracking, celebration, the explain/next
// buttons — is identical, so both paths share this.
...
function applyAnswerResult(isCorrect) {
  ...
  if (isCorrect) {
    ...
    if (currentQuiz.consecutiveWrong === 2) {
      const question = questions[index];
      showKiwiHelper(question.explanation);
    }
  }
...
// same panel as the manual "Stuck? Ask Kiwi" button, just framed as
// Kiwi noticing rather than being asked — and skips straight to a
// real AI hint rather than making the student ask first.
// ------------------------------------------------------------------


async function showKiwiHelper(explanationFallback) {
  const question = currentQuiz.questions[currentQuiz.index];
  const panel = document.getElementById("kiwi-hint-panel");
  const textEl = document.getElementById("kiwi-hint-text");
  const mascotEl = document.getElementById("kiwi-hint-mascot");

  renderKiwiMascotInto(mascotEl, 60);
  panel.classList.remove("hidden");
  textEl.textContent = "Hey, looks like this one's tricky — let me help...";

  const hint = await fetchKiwiHint({
    question: question.q,
...
document.getElementById("kiwi-hint-btn").addEventListener("click", async () => {
  const question = currentQuiz.questions[currentQuiz.index];
  const panel = document.getElementById("kiwi-hint-panel");
  const textEl = document.getElementById("kiwi-hint-text");
  const mascotEl = document.getElementById("kiwi-hint-mascot");
  const btn = document.getElementById("kiwi-hint-btn");
  ...
  textEl.textContent = hintText;
  btn.disabled = false;
  registerMissionEvent("hintsUsed", 1);
});

document.getElementById("kiwi-hint-close-btn").addEventListener("click", () => {
  document.getElementById("kiwi-hint-panel").classList.add("hidden");
});

// The retry-specific "Ask Kiwi for a hint" button, shown only after
