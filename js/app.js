...
// Holds the { subject, lessonIndex } the skip-ahead warning is
// currently asking about, so the modal's own Continue/Go back buttons
// know what to do — kept separate from _pendingLessonStart (which is
// about the concept screen further down the flow, not this warning).
let _pendingSkipAhead = null;

function confirmSkipAheadThenStart(subject, lessonIndex) {
  _pendingSkipAhead = { subject, lessonIndex };
  const mascotEl = document.getElementById("skip-lesson-mascot");
  mascotEl.innerHTML = "";
  // Defensively caught: if rendering the mascot ever throws for any
  // reason, the warning itself (and its buttons) must still work —
  // losing the little mascot graphic is a cosmetic problem, but a
  // stuck, unclickable full-screen overlay with no way out is a much
  // worse one.
  try { renderKiwiMascotInto(mascotEl, 70); } catch (err) { console.warn("Kiwi mascot render failed:", err); }
  document.getElementById("skip-lesson-warning-overlay").classList.remove("hidden");
}

document.getElementById("skip-lesson-continue-btn").addEventListener("click", () => {
  document.getElementById("skip-lesson-warning-overlay").classList.add("hidden");
  if (_pendingSkipAhead) {
    showLessonIntro(_pendingSkipAhead.subject, _pendingSkipAhead.lessonIndex);
    _pendingSkipAhead = null;
  }
});

document.getElementById("skip-lesson-back-btn").addEventListener("click", () => {
  document.getElementById("skip-lesson-warning-overlay").classList.add("hidden");
  _pendingSkipAhead = null;
});

// Belt-and-suspenders: clicking the darkened backdrop itself (outside
// the card) also dismisses the warning, exactly like "Go back". This
// is standard modal behavior anyway, but it specifically matters here
// as a guaranteed way out if either button were ever unresponsive for
// some reason — the backdrop covers the whole screen, so there's
// always something clickable that closes it.
document.getElementById("skip-lesson-warning-overlay").addEventListener("click", (e) => {
  if (e.target.id === "skip-lesson-warning-overlay") {
    document.getElementById("skip-lesson-warning-overlay").classList.add("hidden");
    _pendingSkipAhead = null;
  }
});
