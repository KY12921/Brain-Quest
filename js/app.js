...
function showScreen(id) {
  ...
  document.querySelectorAll(".sidebar-link").forEach(b => b.classList.toggle("active", b.dataset.nav === id));
}

function setTopBarVisible(visible) {
  document.getElementById("top-bar").classList.toggle("hidden", !visible);
  document.getElementById("sidebar-nav").classList.toggle("hidden", !visible);
}
...
async function updateStreak() {
  ...
  const newStreak = currentUserData.lastActiveDate === yesterday ? (currentUserData.streakCount || 0) + 1 : 1;
...
function renderSubjects() {
  ...
  SUBJECTS.forEach(subject => {
    ...
    if (hasRoadmap) {
      ...
      metaText = completedCount === lessons.length ? "Brain Quest complete ✅" : `Lesson ${unlockedCount} of ${lessons.length}`;
    } else {
...
function renderRoadmap(subject) {
  document.getElementById("roadmap-heading").textContent = `${subject.name} Brain Quest`;
  const lessons = LESSONS[subject.id];
  const unlockedCount = (currentUserData.lessonProgress && currentUserData.lessonProgress[subject.id]) || 1;
  ...
  lessons.forEach((lesson, i) => {
    ...
    btn.className = "roadmap-dot" + (isCompleted ? " seal-badge roadmap-dot-done" : "") + (!isReached ? " roadmap-dot-locked" : "");
    btn.innerHTML = isCompleted ? UTIL_ICON_SVG.check : (isReached ? String(i + 1) : UTIL_ICON_SVG.lock);
    btn.disabled = !isReached;
    if (isReached) btn.addEventListener("click", () => showLessonIntro(subject, i));
...
async function finishLesson() {
  ...
  const xpGained = correctCount * xpPerCorrectForMode("lesson");
  ...
  if (lessonIndex === currentUnlocked - 1 && lessonIndex + 1 < totalLessons) {
  ...
  document.getElementById("results-eyebrow").textContent = "Lesson complete";
  ...
  document.getElementById("results-message").textContent = unlockedNext
    ? `${resultMessage(correctCount, questions.length)} Next lesson unlocked!`
