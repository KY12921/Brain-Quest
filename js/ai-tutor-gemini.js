...
// Kiwi's hint — deliberately never reveals the correct answer, just
...
async function fetchKiwiHint({ question, options, subject, gradeLevel }) {
  try {
    const result = await callWorkerFunction("getHint", { question, options, subject, gradeLevel });
    return result.hint || null;
  } catch (err) {
    console.warn("Kiwi's hint unavailable:", err.message || err);
    return null;
  }
}
