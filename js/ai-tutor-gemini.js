...
// Ollie's hint — deliberately never reveals the correct answer, just
// nudges the student's thinking. Falls back to a generic, still-safe
// nudge if the AI is unavailable, rather than showing an error.
async function fetchOllieHint({ question, options, subject, gradeLevel }) {
  try {
    const result = await callWorkerFunction("getHint", { question, options, subject, gradeLevel });
    return result.hint || null;
  } catch (err) {
    console.warn("Ollie's hint unavailable:", err.message || err);
    return null;
  }
}
