...
function getBossQuestions(subjectId, count) {
  ...
  // Boss fights pull ONLY from the hardest (Advanced) tier — this is
  // what actually makes them harder than Trivia, which mixes all
  // three tiers. Previously both drew from the exact same combined
  // pool, so a Pro/Ultra player would see near-identical questions
  // in both modes.
  const advancedTier = chapters[chapters.length - 1] || [];
  const pool = applyInteractiveSetting(advancedTier);
  const result = [];
...
function renderHeartsHud() {
  ...
  if (wrong > 0) {
    hud.classList.remove("hearts-shake");
    void hud.offsetWidth;
    hud.classList.add("hearts-shake");
  }
}

async function finishBossFight(won) {
  const { correctCount, questions, subjectId, subjectName } = currentQuiz;
  const xpGained = (currentQuiz.xpEarned || 0) + (won ? BOSS_WIN_BONUS_XP : 0);
