...
function listenToTeamBattle(battleId) {
  ...
  _activeTeamBattleListener = db.collection("teamBattles").doc(battleId).onSnapshot((doc) => {
    ...
    if (battle.teamA.length === 2 && battle.teamB.length === 2 && battle.status === "waiting") {
...
function renderTeamRoster(battle) {
  ...
  joinABtn.classList.toggle("hidden", onAnyTeam || battle.teamA.length >= 2);
  joinBBtn.classList.toggle("hidden", onAnyTeam || battle.teamB.length >= 2);

  document.getElementById("team-battle-waiting-status").textContent =
    onAnyTeam ? "Waiting for both teams to fill up (2 players each)…" : "Pick a team to join.";
}
