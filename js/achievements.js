...
// tracked (xp, completedSubjects, isPro, rank, coins, ownedCosmetics)
// — no storage needed, just a check() function run live against the
// current user. A handful of categories needed a small new lifetime
// counter that didn't exist before (battles won, perfect lessons,
// total correct answers, coins spent) — those are incremented at the
// relevant point in app.js/duels.js/shop.js.
//
...
// Originally, most achievements here were deliberately reward-free —
// only the ~30 big milestones above paid coins, so those actually felt
// like a stretch goal rather than routine currency. In practice that
// read as "badges don't do anything" rather than "these particular
// badges are extra special," so every achievement now pays something:
// a flat, modest amount for the ones that were reward-free before,
// leaving the explicit table above untouched so the big milestones
// still clearly pay more.
const DEFAULT_ACHIEVEMENT_REWARD_COINS = 50;

ACHIEVEMENTS.forEach(a => {
  a.reward = ACHIEVEMENT_REWARDS[a.id] || { coins: DEFAULT_ACHIEVEMENT_REWARD_COINS };
});
