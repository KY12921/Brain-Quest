...
async function renderClansScreen() {
  ...
  if (!hasClan) {
    const isUltra = !!currentUserData.isUltra;
    document.getElementById("create-clan-free-btn").classList.toggle("hidden", !isUltra);
    document.getElementById("create-clan-btn").classList.toggle("hidden", isUltra);
    document.getElementById("create-clan-cash-btn").classList.toggle("hidden", isUltra);
  }
...
async function renderMyClan() {
  ...
  try {
    ...
    // Join requirements are owner-only to view or change — everyone
    // else just experiences them as "this clan needs level 5" if they
    // try to join and don't meet it (see joinClan's error handling).
    const isOwner = currentUser && clan.ownerUid === currentUser.uid;
    document.getElementById("clan-owner-settings").classList.toggle("hidden", !isOwner);
    if (isOwner) {
      document.getElementById("clan-min-level-input").value = clan.minLevel || 0;
      document.getElementById("clan-min-lessons-input").value = clan.minLessonsCompleted || 0;
    }
  } catch (err) {
...
document.getElementById("create-clan-btn").addEventListener("click", async () => {
  ...
  if ((currentUserData.coins || 0) < 10000) {
    // Same pattern as the Shop's insufficient-funds handling
    // (purchaseShopItem in shop.js): don't just leave the person at a
    // dead end with an accurate-but-unhelpful error — send them
    // straight to where coin packs and Pro/Ultra's earning bonuses
    // actually live, so there's a next step, not just a wall.
    statusEl.textContent = "Creating a clan costs 10,000 coins — taking you to get more.";
...
// Ultra members' free path — same backend endpoint as the coin path
// above (the server is the source of truth on whether it's actually
// free), just without any client-side coin deduction, since Ultra
// users are charged nothing.
document.getElementById("create-clan-free-btn").addEventListener("click", async () => {
  const name = document.getElementById("clan-name-input").value.trim();
  const statusEl = document.getElementById("clans-status");
  const btn = document.getElementById("create-clan-free-btn");
  statusEl.textContent = "";
...
// Real-money alternative to the coin path — redirects to Stripe
// Checkout. The clan itself only gets created after Stripe confirms
// payment (handled server-side by the webhook), not here — this just
// starts checkout and sends the browser to Stripe's page.
document.getElementById("create-clan-cash-btn").addEventListener("click", async () => {
  const name = document.getElementById("clan-name-input").value.trim();
  const statusEl = document.getElementById("clans-status");
  const btn = document.getElementById("create-clan-cash-btn");
  statusEl.textContent = "";
  ...
  btn.disabled = true;
  try {
    const result = await callWorkerFunction("createClanCheckoutSession", {
      clanName: name,
      successUrl: window.location.origin + window.location.pathname + "?clanPurchase=success",
      cancelUrl: window.location.origin + window.location.pathname
    });
    window.location.href = result.url;
  } catch (err) {
    statusEl.textContent = err.message || "Couldn't start checkout right now.";
    btn.disabled = false;
  }
});
...
document.getElementById("save-clan-settings-btn").addEventListener("click", async () => {
  const statusEl = document.getElementById("clans-status");
  const minLevel = parseInt(document.getElementById("clan-min-level-input").value, 10) || 0;
  const minLessonsCompleted = parseInt(document.getElementById("clan-min-lessons-input").value, 10) || 0;
  statusEl.textContent = "";
  try {
    const result = await callWorkerFunction("updateClanSettings", { minLevel, minLessonsCompleted });
    statusEl.textContent = result.message || "Clan settings updated.";
  } catch (err) {
    statusEl.textContent = err.message || "Couldn't update clan settings.";
  }
});
...
async function renderClanLeaderboard(metric) {
  ...
  try {
    ...
    listEl.innerHTML = rows.map((c, i) => {
      const isMine = currentUserData.clanId === c.id;
      return `
      <div class="leaderboard-row ${isMine ? "leaderboard-row-me" : ""}">
        ...
        ${isMine ? "" : `<button class="btn btn-ghost clan-leaderboard-join-btn" data-clan-id="${c.id}">Join</button>`}
      </div>
    `;
    }).join("");
    listEl.querySelectorAll(".clan-leaderboard-join-btn").forEach(btn => {
      btn.addEventListener("click", () => joinClan(btn.dataset.clanId));
    });
  } catch (err) {
