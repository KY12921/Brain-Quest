...
// Items shown ONLY in the rotating shop — never in the permanent
// grid below. This is what makes the daily rotation feel like an
// actual limited-time selection rather than just a discounted
// reshuffle of things already permanently available.
const ROTATING_EXCLUSIVE_ITEMS = [
  { id: "nameplate-frost", category: "Nameplates", name: "Frost Nameplate", description: "An icy blue glow — rotating shop exclusive.",
    cost: 500, cosmeticCategory: "nameplates", cosmeticId: "frost" },
  { id: "nameplate-ember", category: "Nameplates", name: "Ember Nameplate", description: "A fiery orange-to-gold gradient — rotating shop exclusive.",
    cost: 550, cosmeticCategory: "nameplates", cosmeticId: "ember" },
  { id: "nameplate-royal", category: "Nameplates", name: "Royal Nameplate", description: "A regal purple glow — rotating shop exclusive.",
    cost: 500, cosmeticCategory: "nameplates", cosmeticId: "royal" },
  { id: "nameplate-ocean", category: "Nameplates", name: "Ocean Nameplate", description: "A teal-to-blue gradient — rotating shop exclusive.",
    cost: 550, cosmeticCategory: "nameplates", cosmeticId: "ocean" },
  { id: "nameplate-mint", category: "Nameplates", name: "Mint Nameplate", description: "A soft minty glow — rotating shop exclusive.",
    cost: 450, cosmeticCategory: "nameplates", cosmeticId: "mint" },
  { id: "nameplate-plum", category: "Nameplates", name: "Plum Nameplate", description: "A magenta-to-pink gradient — rotating shop exclusive.",
    cost: 550, cosmeticCategory: "nameplates", cosmeticId: "plum" },
  { id: "nameplate-crimson", category: "Nameplates", name: "Crimson Nameplate", description: "A deep red glow — rotating shop exclusive.",
    cost: 500, cosmeticCategory: "nameplates", cosmeticId: "crimson" },
  { id: "nameplate-lavender", category: "Nameplates", name: "Lavender Nameplate", description: "A soft purple glow — rotating shop exclusive.",
    cost: 450, cosmeticCategory: "nameplates", cosmeticId: "lavender" },
  { id: "nameplate-steel", category: "Nameplates", name: "Steel Nameplate", description: "A cool metallic gray-blue — rotating shop exclusive.",
    cost: 450, cosmeticCategory: "nameplates", cosmeticId: "steel" },
  { id: "nameplate-sunset", category: "Nameplates", name: "Sunset Nameplate", description: "An orange-to-pink gradient — rotating shop exclusive.",
    cost: 550, cosmeticCategory: "nameplates", cosmeticId: "sunset" },
  { id: "nameplate-jade", category: "Nameplates", name: "Jade Nameplate", description: "A rich green glow — rotating shop exclusive.",
    cost: 500, cosmeticCategory: "nameplates", cosmeticId: "jade" },
  { id: "nameplate-cosmic", category: "Nameplates", name: "Cosmic Nameplate", description: "A purple-to-pink galaxy gradient — rotating shop exclusive.",
    cost: 600, cosmeticCategory: "nameplates", cosmeticId: "cosmic" },
  { id: "nameplate-cherry", category: "Nameplates", name: "Cherry Nameplate", description: "A bright pink-red glow — rotating shop exclusive.",
    cost: 500, cosmeticCategory: "nameplates", cosmeticId: "cherry" },
  { id: "nameplate-arctic", category: "Nameplates", name: "Arctic Nameplate", description: "A pale icy-white glow — rotating shop exclusive.",
    cost: 500, cosmeticCategory: "nameplates", cosmeticId: "arctic" },
  { id: "nameplate-amber", category: "Nameplates", name: "Amber Nameplate", description: "A warm golden-orange glow — rotating shop exclusive.",
    cost: 500, cosmeticCategory: "nameplates", cosmeticId: "amber" },
  { id: "decoration-flame", category: "Decorations", name: "Flame Decoration", description: "A small flickering flame icon — rotating shop exclusive.",
    cost: 300, cosmeticCategory: "decorations", cosmeticId: "flame" },
  { id: "decoration-leaf", category: "Decorations", name: "Leaf Decoration", description: "A simple green leaf icon — rotating shop exclusive.",
    cost: 275, cosmeticCategory: "decorations", cosmeticId: "leaf" }
];

function getRotatingShopItems() {
  const today = localDateString();
  const rng = seededRandom("rotating-shop-" + today);
  const eligible = ROTATING_EXCLUSIVE_ITEMS;
...
let _currentRotatingItems = []; // set by renderRotatingShop — lets the
                                 // preview click handler below look up
                                 // a rotating item (with its discount),
                                 // since these aren't part of SHOP_ITEMS.

// The rotation itself changes based on the calendar date string (see
// seededRandom("rotating-shop-" + today) above) — meaning it actually
// refreshes at local midnight. This countdown just makes that visible
// rather than leaving people to guess when "tomorrow" arrives. Ticks
// every second while the shop screen is open; cleared and restarted
// each time renderRotatingShop runs so navigating away and back never
// stacks up multiple competing intervals.
let _rotatingShopCountdownInterval = null;

function startRotatingShopCountdown() {
  clearInterval(_rotatingShopCountdownInterval);
  const el = document.getElementById("rotating-shop-countdown");
  if (!el) return;
  ...
  const tick = () => {
    ...
    // Crossing midnight while the shop is sitting open — reroll the
    // items rather than leave yesterday's picks showing with a
    // 00:00:00 countdown that never moves again.
    if (msLeft <= 1000) renderRotatingShop();
  };
  tick();
  _rotatingShopCountdownInterval = setInterval(tick, 1000);
}
...
function renderRotatingShop() {
  ...
  _currentRotatingItems = items;

  container.innerHTML = items.map(item => {
    const isOwned = isCosmeticOwned(item, owned);
    const affordable = (currentUserData.coins || 0) >= item.discountedCost;
    return `
      <div class="shop-item rotating-shop-item${!affordable && !isOwned ? " shop-item-unaffordable" : ""}" data-item-id="${item.id}" data-preview="true">
        ...
        <p class="shop-item-price-row">
          ...
          <span class="shop-item-price-divider">or</span>
          <span class="shop-item-price-cash">$${(item.discountedCost / 100).toFixed(2)}</span>
        </p>
  ...
  // Same "click the card to preview, click the button to buy" pattern
  // as the permanent shop grid below — this was missing entirely
  // here, so clicking a rotating item card did nothing.
  container.querySelectorAll(".rotating-shop-item").forEach(card => {
    card.addEventListener("click", (e) => {
      if (e.target.closest(".rotating-shop-buy-btn")) return;
      const item = _currentRotatingItems.find(i => i.id === card.dataset.itemId);
      if (item) showItemPreview(item, item.discountedCost);
    });
  });
...
const SHOP_ITEMS = [
  ...
  { id: "temp-pro-1day", category: "Boosts", name: "Pro for 1 Day", description: "Every Pro perk for 24 hours.",
    cost: 5000, apply: () => applyTempPro(24 * 60 * 60 * 1000) },
...
function showItemPreview(item, previewCost) {
  // previewCost lets a caller (the rotating shop, for its 20%-off
  // items) show and charge the actual discounted price here instead
  // of item.cost — without it, opening a rotating item's preview
  // would silently quote and charge full price.
  const effectiveCost = typeof previewCost === "number" ? previewCost : item.cost;
  ...
  buyBtn.textContent = isOwned ? "Owned" : `Buy — ${effectiveCost} coins`;
  buyBtn.disabled = isOwned;
  buyBtn.onclick = () => {
    modal.classList.add("hidden");
    purchaseShopItem(item.id, effectiveCost);
  };
  ...
  //
  // Deliberately uses item.cost (full price), not effectiveCost, for
  // the real-money button specifically: the rotating shop's 20% off
  // is a coin-price discount only, applied client-side when spending
  // coins. The Stripe checkout session is priced server-side purely
  // by itemId (see handleCreateItemCheckoutSession), with no concept
  // of today's rotation discount — quoting the discounted price here
  // would promise something Stripe wouldn't actually charge.
...
async function purchaseShopItem(itemId, overrideCost) {
  // Rotating-exclusive items (see ROTATING_EXCLUSIVE_ITEMS below) are
  // deliberately never part of SHOP_ITEMS — they're only ever shown
  // via getRotatingShopItems(). Looking in SHOP_ITEMS alone meant
  // buying one of those specific items found nothing and silently did
  // nothing at all: no purchase, no error, no feedback — exactly the
  // "can't buy from the daily shop" symptom. Falling back to the
  // rotating list (recomputed fresh here, not just the cached
  // _currentRotatingItems from the last render, so this also works
  // correctly if called right after a render elsewhere) fixes that.
  const item = SHOP_ITEMS.find(i => i.id === itemId) || getRotatingShopItems().find(i => i.id === itemId);
  ...
  showPurchaseToast(item.name);
  renderShopScreen();
}

// A brief, hard-to-miss confirmation on top of the plain status text
// above — that text line is easy to miss if it's scrolled out of
// view, especially on a long shop page. Auto-dismisses on its own so
// it never needs a close button.
let _purchaseToastTimeout = null;
function showPurchaseToast(itemName) {
  const toast = document.getElementById("purchase-toast");
  if (!toast) return;
  document.getElementById("purchase-toast-message").textContent = `Purchased ${itemName}!`;
  toast.classList.remove("hidden");
  clearTimeout(_purchaseToastTimeout);
  _purchaseToastTimeout = setTimeout(() => toast.classList.add("hidden"), 2200);
}
