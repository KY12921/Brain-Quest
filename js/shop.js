...
// ------------------------------------------------------------------
const DAILY_SPIN_SLOTS = [
  { type: "nothing", label: "Nothing" },
  { type: "xpBoost", label: "1.5x XP Boost" },
  { type: "nothing", label: "Nothing" },
...
function renderDailySpinner() {
  const slotsEl = document.getElementById("daily-spin-slots");
  const btn = document.getElementById("daily-spin-btn");
  const resultEl = document.getElementById("daily-spin-result");
  resultEl.textContent = "";

  slotsEl.innerHTML = DAILY_SPIN_SLOTS.map((slot, i) => `<div class="daily-spin-slot" id="daily-spin-slot-${i}">${slot.label}</div>`).join("");
...
async function spinDailyWheel() {
  ...
  const winningIndex = Math.floor(Math.random() * DAILY_SPIN_SLOTS.length);
  const winningSlot = DAILY_SPIN_SLOTS[winningIndex];

  // Brief highlight animation cycling through slots before landing on
  // the actual (already-decided) result — purely visual, the outcome
  // is fixed the moment the spin starts, not influenced by the animation.
  const totalSteps = 16 + winningIndex;
  for (let step = 0; step < totalSteps; step++) {
    const idx = step % DAILY_SPIN_SLOTS.length;
    document.querySelectorAll(".daily-spin-slot").forEach(el => el.classList.remove("daily-spin-slot-active"));
    const el = document.getElementById(`daily-spin-slot-${idx}`);
    if (el) el.classList.add("daily-spin-slot-active");
    await new Promise(r => setTimeout(r, 60 + step * 4)); // slows down toward the end
  }
