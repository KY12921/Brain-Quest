...
async function finishTutorSession() {
  ...
  // next time. No live AI call — just picking from the harder curated
  // tier, invisibly, based on performance.
  let advanced = false;
  if (correctCount > 0 && tutorTier < CHAPTER_NAMES.length - 1) {
    ...
    if (currentUser) {
      await db.collection("users").doc(currentUser.uid).update({
        [`tutorLevel.${subjectId}`]: tutorTier + 1
      });
    }
  }
  ...
  document.getElementById("results-message").textContent = correctCount === questions.length
    ? (advanced ? `Nice — next time you'll get a ${CHAPTER_NAMES[tutorTier + 1]} question.` : "Nice — you've got this one down.")
