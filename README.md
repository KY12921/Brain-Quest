...
**Current build: 1 — 2026-09-06** (also shown live in the app itself, on the
sign-in screen and in Settings — check that number against what was most
recently delivered to confirm you're running the build you think you are,
rather than an older download.)

A gamified study app: sign in, pick a subject, answer questions, earn XP —
with Daily Missions, a Leaderboard, Duels, and a Pro tier.
...
- **Real Gemini calls are scoped to the AI Tutor only, not every
  question everywhere.** Quests, Duels, and Trivia still
...
- **The old 3-tier chapter system is now unused for Quests, but still
  intact and used elsewhere.** Since version5 now covers all 10
  subjects, no subject falls back to the old chapters anymore for
  Quests — but that data (`QUESTION_BANK`) wasn't deleted, since
  Trivia, the AI Tutor, and Duels all still pull from it
...
- **No live AI-generated questions.** Same as before — GitHub Pages is
  ...
  (Quests, Duels) draw from the curated bank in
  `js/questions.js`.
...
```
version5/
├── index.html               All screens: auth, quests, quiz, missions, leaderboard,
...
├── js/kiwi-mascot.js        Kiwi the mascot, flat 2D SVG (previously called "Ollie" and rendered in
│                             real 3D via Three.js — renamed and simplified; see js/build-version.js
│                             below for why an old README/build mismatch like that is now easy to catch)
├── js/build-version.js      Visible build identifier (sign-in screen + Settings) — bump APP_BUILD
│                             on every delivery so it's checkable which build is actually live
