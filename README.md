...
- **Interactive questions**: a few questions (3 total, in Math) are
  drag-based instead of multiple choice — a balance scale for solving
  simple equations, and a draggable line grapher for slope. Built with
  Pointer Events so dragging works on touch devices, not just mouse.
  Most questions are still multiple choice on purpose — these are
...
- **Go Pro**: a $5.99/month tier removing ads and unlocking unlimited
  AI Tutor use. See the honest note below — there's no real payment
  processing yet.
...
- **Only 3 interactive questions exist so far.** Building a genuinely
  good drag interaction (with real cross-device pointer handling,
  snapping, and visual feedback) takes real work per question type,
  so this is a proof-of-concept sprinkle, not full coverage. Adding
  more is straightforward now that the two widget types exist in
  `js/interactive.js` — new questions of type `"balance"` or
  `"slope-drag"` just need question data in `js/questions.js` and
  they'll work automatically; a third widget type would need a
  render function following the same pattern.
...
### 5. Create the composite index the ranked leaderboard needs
The Ranked Leaderboard queries "all users in my rank, sorted by weekly
XP" — a `where` filter combined with an `orderBy` on a different
field. Firestore requires a composite index for this exact combination
before the query will work. The easiest way to create it: open the
app, go to the **Leaderboard** tab, and open your browser's developer
console (F12 → Console). Firestore's error message there includes a
direct link that auto-creates the correct index — click it, wait about
a minute for it to build, then reload. Until this index exists, the
leaderboard will show "Couldn't load the leaderboard right now."

### 6. Security rules (important — read this if you set rules beyond test mode)
