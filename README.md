...
- **The roadmap is only fully built for Math and Science.** History,
  Geography, English, Computer Science, and Economics still use the
  old 3-tier chapter system for their Quests — nothing broke, they
  just haven't been converted to the new 10-lesson format yet. The
  `LESSONS` object in `js/questions.js` is where that content lives;
  adding it for another subject means defining 10 topic names and
  writing 5 questions each, following the same shape as Math/Science.
