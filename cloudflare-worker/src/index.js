...
import { getFirestoreDocument, updateFirestoreDocument, incrementFirestoreField, findFirestoreDocByField } from "./firestore.js";
...
async function handleCreateCheckoutSession(request, env) {
  ...
  let sessionConfig;
  if (productType === "pro") {
    sessionConfig = { mode: "subscription", line_items: [{ price: env.STRIPE_PRICE_ID_PRO, quantity: 1 }] };
  } else if (productType === "ultra") {
    sessionConfig = { mode: "subscription", line_items: [{ price: env.STRIPE_PRICE_ID_ULTRA, quantity: 1 }] };
  } else if (productType === "coins1000") {
    sessionConfig = { mode: "payment", line_items: [{ price: env.STRIPE_PRICE_ID_COINS1000, quantity: 1 }] };
  } else {
    return errorJson("Unknown productType: " + productType, 400);
  }
...
async function handleStripeWebhook(request, env) {
  ...
  let event;
  try {
    // constructEventAsync + createSubtleCryptoProvider is the
    // edge-compatible verification path — the synchronous
    // constructEvent relies on Node's crypto module, which doesn't
    // exist in Workers.
  ...
  if (event.type === "checkout.session.completed") {
    ...
    if (userId && productType === "coins1000") {
      await incrementFirestoreField(env.FIREBASE_PROJECT_ID, env.FIREBASE_SERVICE_ACCOUNT, "users", userId, "coins", 1000);
