...
// option is correct.
// Ollie's hint: a Socratic nudge, not the answer. Two layers of
...
Rules: don't say which option is correct, don't perform the final calculation or arrive at the answer yourself, and don't restate any of the answer options as part of your hint. Warm and encouraging tone, no headers or lists, no more than 3 sentences.`;
...
"correct" is the 0-based index of the right option within "options". Every question needs a real, accurate explanation.`;
...
Write a short, encouraging summary (3-5 sentences) of how they did overall, and specifically call out which topic(s) they should practice more, based on where their accuracy was lowest. Be specific about the weak topic(s), not generic. No headers or bullet points, just warm, direct sentences.`;

  try {
    const text = await callGemini(geminiKey, [{ role: "user", parts: [{ text: prompt }] }], { maxOutputTokens: 300, temperature: 0.7 });
...
async function handleCreateClan(request, projectId, serviceAccount) {
  ...
  if ((me.coins || 0) < 10000) return errorJson("Creating a clan costs 10,000 coins — you don't have enough yet.", 400);
  ...
  await updateFirestoreDocument(projectId, serviceAccount, "users", auth.uid, {
    coins: (me.coins || 0) - 10000,
    lifetimeCoinsSpent: (me.lifetimeCoinsSpent || 0) + 10000,
    clanId: clanRef.id,
    clanName: name
  });

  return json({ success: true, clanId: clanRef.id, message: `Clan "${name}" created!` });
}
...
// (lessons, boss fights, trivia, duels, team battles, study mode) to
